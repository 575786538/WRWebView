...
#pragma mark - 创建按钮
    ...
    UIAlertAction *ActionSacn = [UIAlertAction actionWithTitle:@"识别二维码"
                                                       ...
                                                       handler:^(UIAlertAction * _Nonnull action)
                                 {
                                     ...
                                     //返回二维码信息
                                     [[NSNotificationCenter defaultCenter] postNotificationName:@"NotifeLoadRequest" object:nil userInfo:@{@"LoadQRCodeUrl":url.absoluteString}];
                                 }];
...
- (void)LoadImageFromURL:(NSURL *)URLString
{
    ...
    if (!image) {
        [UIAlertController WRlertWithTitle:@"提示" message:@"下载失败" completion:nil];
        return;
    }
...
- (void)saveImageFinished:(UIImage *)image
{
    ...
    [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
        ...
        if (success){
            ...
            [UIAlertController WRlertWithTitle:@"提示" message:@"已经保存到相册" completion:nil];
        }else{
            
            [UIAlertController WRlertWithTitle:@"提示" message:@"保存失败" completion:nil];
        }
    }];
}
